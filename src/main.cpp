...
#if defined(NDEBUG)
#error "Vbuck cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 100 * COIN;
    } else if (nHeight >= 30001 && nHeight <= 45000) {
        nSubsidy = 130 * COIN;
    } else if (nHeight >= 45001 && nHeight <= 55000) {
        nSubsidy = 150 * COIN;
    } else if (nHeight >= 55001 && nHeight <= 65000) {
        nSubsidy = 170 * COIN;
    } else if (nHeight >= 65001 && nHeight <= 75000) {
        nSubsidy = 200 * COIN;
    } else if (nHeight >= 75001 && nHeight <= 85000) {
        nSubsidy = 220 * COIN;
    } else if (nHeight >= 85001) {
        nSubsidy = 50.25 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // VBUCK: We use certain sporks during IBD, so check to see if they are
